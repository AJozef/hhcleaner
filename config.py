...
def get_default_profile() -> str:
    """Возвращает имя профиля по умолчанию.

    Приоритет источников:
        1. HH_PROFILE — env-переменная (например, в .env или окружении ОС).
        2. config.toml [defaults] → profile — персистентный конфиг.
        3. «default» — жёсткий фолбэк.

    Импортируем app_config лениво, чтобы избежать циклического импорта
    (app_config сам импортирует APP_DIR из этого модуля).
    """
    from_env = os.environ.get("HH_PROFILE", "").strip()
    if from_env:
        return from_env
    try:
        import app_config  # pylint: disable=import-outside-toplevel
        cfg = app_config.load()
        from_cfg = str(cfg.get("profile", "")).strip()
        if from_cfg:
            return from_cfg
...
def get_user_data_dir(profile: str = "default") -> str:
    """Путь к каталогу профиля браузера.

    «default» → обратно совместимое ~/.hhcleaner/userdata.
    Именованные профили → ~/.hhcleaner/profiles/<name>/.
    Переопределяется HHCLEANER_HOME.
    """
    if profile == "default":
        return os.path.join(APP_DIR, "userdata")
    return os.path.join(APP_DIR, "profiles", profile)
